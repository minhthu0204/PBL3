...
namespace electronic_scale
{
    public partial class SaleManager : Form
    {
        ...
        string strCon = "Data Source=DESKTOP-O6OP19G\\WINCCPLUSMIG2014;Initial Catalog=saleManager;Integrated Security=True";
        ...
        private void txtQR_TextChanged(object sender, EventArgs e)
        {
            if (txtQR.Text != "")
            {
                ...
                using (SqlCommand cmd = new SqlCommand("select * from product where Barcode ='" + maBarcode + "'", sqlCon))         // gui truy van vao ket noi
                {
                ...
                {
                    using (SqlCommand sqlCmd2 = new SqlCommand("Insert into Sale(Macode,TenSanPham,Soluong,Giaban,Thanhtien,Ngaymuahang) values(@code,@tensanpham,@soluong,@giaban,@thanhtien,@ngaymuahang)", sqlCon))
                    {
        ...
        private void khoi_tao_bang()
        {
            ...
            string[] header_name = new string[NumberColumn] { "BarCode", "Name", "Unit", "Quantity", "Price", "Cost", "DateTime" };
            int[] width_col = new int[NumberColumn] { 150, 120, 120, 120, 120, 120, 150 };
        ...
        private void btnInhoadon_Click(object sender, EventArgs e)
        {
            if (txtMaKH.Text.Length > 0 && txtTenKH.Text.Length > 0)
            {
                ...
                frm2.reportViewer1.LocalReport.ReportEmbeddedResource = "electronic_scale.Report1.rdlc";
                frm2.ShowDialog();
            }
            else
            {
